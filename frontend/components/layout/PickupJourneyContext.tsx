            ...
            <div>
                ...
                <p className="mt-1 font-semibold text-[#241715]">
                    {mismatchedCart ? "Cart belongs to another branch — review before checkout"
                        : slot && selection ? `${pickupDateLabel(selection.date)} · ${formatBusinessTime(slot.startTime)}–${formatBusinessTime(slot.endTime)} IST`
                        : date ? `${pickupDateLabel(date)} · Time not chosen` : "Choose a pickup time at checkout"}
                </p>
